...
subdirs = glob("./out/CSA/hete/*/") #hete or homo

groups=['CSA-M','CSA-Q','CSSA-M','ITE-NUC']
...
Data1 = pd.concat(Ds)
Data1['gmm_cat'] = pd.Categorical(Data1['gmm'], categories=list(sorted(gmms)))

subdirs = glob("./out/CSSA/hete/*/") #hete or homo
...
Data2 = pd.concat(Ds)
Data2['gmm_cat'] = pd.Categorical(Data2['gmm'], categories=list(sorted(gmms)))
...
p = ggplot(Data, aes(x='gmm_cat', y=t, fill='group')) +\
    ...
    labs(x=r'Confounding strength ($\Gamma$)',y=t) +\
        ylim([0.45,1.0]) +\
    scale_fill_manual({'CSA-M':palette[0], 'CSA-Q':palette[2], 'CSSA-M':palette[4], 'ITE-NUC':palette[3]})

p += geom_hline(yintercept=1-alpha,linetype='dashed',size=1, color='red')

print(p)
plt.tight_layout()
save_as_pdf_pages([p+ theme(figure_size=(10, 5))],'hete-coverage.pdf')
#%%
#### Length plot #######
alpha=0.2
t="Interval-length"
...
p = ggplot(Data,aes(x='gmm',y='center',group='group',colour='group')) + \
        ...
        scale_color_manual({'CSA-M':palette[0], 'CSA-Q':palette[2],'CSSA-M':palette[4], 'ITE-NUC':palette[3]})

ggplot.draw(p)
save_as_pdf_pages([p+ theme(figure_size=(10, 5))],'hete-interval.pdf')

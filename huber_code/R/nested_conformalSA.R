nested_conformalSA <- function(X, Y1, Y0, T, gmm,
  ...
  # 使用训练集数据(group1)，分别针对Y0和Y1拟合两个模型
  obj_treated <- conformal_SA(X[group1id, , drop=FALSE], Y1[group1id], gmm, type = type,outfun=outfun, nested=TRUE)
  obj_control <- conformal_SA(X[group1id, , drop=FALSE], Y0[group1id], gmm, type = type,outfun=outfun, nested=TRUE)

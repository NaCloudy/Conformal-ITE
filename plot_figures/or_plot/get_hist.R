# 定义获取直方图的函数
get_hist <- function(X1, A_all, Y_all, file_name) {
  library("devtools")
  devtools::load_all(".")
  psfun <- Boosting
  n <- length(Y_all)
  ...
  print(summary(R))
  png(paste("figures/OR/",file_name,"_hist.png", sep = ""))
  hist(R, breaks = seq(1, 4, 0.5), prob=TRUE, xlab='(Adjusted) odds ratio',main=file_name,
